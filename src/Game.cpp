...
Game::Game(const int screenWidth, const int screenHeight, const std::string& title)
    : turn(TurnState::PLAYER_TURN), needs_dice_roll(true), transition_timer(0.0f), current_level(1) {
    SetConfigFlags(FLAG_WINDOW_RESIZABLE);
    InitWindow(screenWidth, screenHeight, title.c_str());
    SetTargetFPS(120);
    assets = LoadGameAssets();
    
    // Create render texture at fixed resolution
    render_texture = LoadRenderTexture(baseScreenWidth, baseScreenHeight);
...
void Game::draw() {
    ...
    DrawTexturePro(assets.background, {0, 0, static_cast<float>(assets.background.width), static_cast<float>(assets.background.height)}, {0, 0, static_cast<float>(baseScreenWidth), static_cast<float>(baseScreenHeight)}, {0, 0}, 0, WHITE);

    // Player and Enemy Info
    draw_text_with_shadow(player.name, 20, 20, 45.0f, WHITE, assets.font);
    std::string player_hp_text = "HP: " + std::to_string(player.health) + "/" + std::to_string(player.max_health);
    draw_text_with_shadow(player_hp_text, 20, 50, 36.0f, WHITE, assets.font);

    const Texture2D player_texture = (player.health <= player.max_health / 2) ? assets.player_hurt : assets.player;
    const float sprite_height = (static_cast<float>(baseScreenHeight) / 2.5f);
    const float sprite_width = sprite_height;
    const float y_pos = static_cast<float>(baseScreenHeight) / 2.0f - sprite_height / 2.0f;
    DrawTexturePro(player_texture, {0, 0, static_cast<float>(player_texture.width), static_cast<float>(player_texture.height)}, {40, y_pos, sprite_width, sprite_height}, {0, 0}, 0, WHITE);

    // Enemy Sprite
    const float enemy_sprite_scale = current_enemy->get_sprite_scale();
    const float enemy_sprite_height = (static_cast<float>(baseScreenHeight) / 2.5f) * enemy_sprite_scale;
    const float enemy_sprite_width = enemy_sprite_height; // Assuming square sprites
    const float enemy_y_pos = static_cast<float>(baseScreenHeight) / 2.0f - enemy_sprite_height / 2.0f;
    const float enemy_x_pos = static_cast<float>(baseScreenWidth) - enemy_sprite_width - 40; // Position on the right
    current_enemy->draw(assets, 1.0f, enemy_x_pos, enemy_y_pos, enemy_sprite_width, enemy_sprite_height);

    Vector2 enemy_name_size = MeasureTextEx(assets.font, current_enemy->name.c_str(), 45.0f, 2);
    const float enemy_name_x = static_cast<float>(baseScreenWidth) - enemy_name_size.x - 20;
    draw_text_with_shadow(current_enemy->name, enemy_name_x, 20, 45.0f, WHITE, assets.font);
    const std::string enemy_hp_text = "HP: " + std::to_string(current_enemy->health) + "/" + std::to_string(current_enemy->max_health);
    Vector2 enemy_hp_size = MeasureTextEx(assets.font, enemy_hp_text.c_str(), 36.0f, 2);
    const float enemy_hp_x = static_cast<float>(baseScreenWidth) - enemy_hp_size.x - 20;
    draw_text_with_shadow(enemy_hp_text, enemy_hp_x, 50, 36.0f, WHITE, assets.font);

    switch (turn) {
        case TurnState::PLAYER_TURN: {
            const float dice_size = 128.0f;
            const float dice_gap = 10.0f;
            ...
            const float start_x = (static_cast<float>(baseScreenWidth) - total_dice_width) / 2.0f;

            for (size_t i = 0; i < current_dice.size(); ++i) {
                DrawTexturePro(assets.dice_tileset, current_dice[i].source_rect, {start_x + static_cast<float>(i) * (dice_size + dice_gap), static_cast<float>(baseScreenHeight) / 2.0f - (dice_size / 2.0f), dice_size, dice_size}, {0,0}, 0, WHITE);
            }

            const float base_y = static_cast<float>(baseScreenHeight) - 120.0f;
            const float line_height = 28.0f;
            draw_text_with_shadow("Abilities (Space to Skip):", 20, base_y, 36.0f, WHITE, assets.font);
            ...
            for (size_t i = 0; i < player.abilities.size(); ++i) {
                ...
                draw_text_with_shadow(ability_line, 20, base_y + (static_cast<float>(i) + 1) * line_height, 36.0f, WHITE, assets.font);
            }
            break;
        }
        case TurnState::ENEMY_TURN: {
            const auto msg = "Enemy is thinking...";
            auto [x, y] = MeasureTextEx(assets.font, msg, 45.0f, 2);
            draw_text_with_shadow(msg, (static_cast<float>(baseScreenWidth) - x) / 2.0f, static_cast<float>(baseScreenHeight) / 2.0f, 45.0f, WHITE, assets.font);
            break;
        }
        case TurnState::LEVEL_COMPLETE: {
            auto [x, y] = MeasureTextEx(assets.font, game_over_message.c_str(), 75.0f, 2);
            draw_text_with_shadow(game_over_message, (static_cast<float>(baseScreenWidth) - x) / 2.0f, static_cast<float>(baseScreenHeight) / 2.0f - 20, 75.0f, YELLOW, assets.font);
            break;
        }
        case TurnState::GAME_OVER: {
            const auto restart_msg = "Press Enter to Restart";
            auto [x, y] = MeasureTextEx(assets.font, game_over_message.c_str(), 75.0f, 2);
            auto [x2, y2] = MeasureTextEx(assets.font, restart_msg, 45.0f, 2);
            draw_text_with_shadow(game_over_message, (static_cast<float>(baseScreenWidth) - x) / 2.0f, static_cast<float>(baseScreenHeight) / 2.0f - 20, 75.0f, YELLOW, assets.font);
            draw_text_with_shadow(restart_msg, (static_cast<float>(baseScreenWidth) - x2) / 2.0f, static_cast<float>(baseScreenHeight) / 2.0f + 30, 45.0f, WHITE, assets.font);
            break;
        }
    }

    auto [x, y] = MeasureTextEx(assets.font, feedback_message.c_str(), 30.0f, 2);
    draw_text_with_shadow(feedback_message, (static_cast<float>(baseScreenWidth) - x) / 2.0f, static_cast<float>(baseScreenHeight) - 40.0f, 30.0f, YELLOW, assets.font);

    EndTextureMode();

    // Draw to screen with aspect-ratio-preserving scaling
    BeginDrawing();
    ClearBackground(BLACK);
    
    // Calculate scale factor that preserves aspect ratio without clipping
    const float scale_x = static_cast<float>(GetScreenWidth()) / static_cast<float>(baseScreenWidth);
    const float scale_y = static_cast<float>(GetScreenHeight()) / static_cast<float>(baseScreenHeight);
    const float scale = std::min(scale_x, scale_y);  // Use min to prevent clipping
    
    // Calculate centered position
    const float scaled_width = baseScreenWidth * scale;
    const float scaled_height = baseScreenHeight * scale;
    const float offset_x = (GetScreenWidth() - scaled_width) / 2.0f;
    const float offset_y = (GetScreenHeight() - scaled_height) / 2.0f;
    
    // Draw the render texture to the screen
    DrawTexturePro(render_texture.texture, 
                   {0, 0, static_cast<float>(baseScreenWidth), static_cast<float>(-baseScreenHeight)},
...
void Game::draw_text_with_shadow(const std::string& text, const float x, const float y, const float font_size, const Color color, const Font& font) const {
    DrawTextEx(font, text.c_str(), {x + 2, y + 2}, font_size, 2, DARKGRAY);
